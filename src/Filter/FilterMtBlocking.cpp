...
void filter_mt_blocking(Image &src, Image &dst, const double *kernel, int N) {
    ...
    const int block_row_size = static_cast<int>((src.height - 1) / num_threads + 1);

    const int block_size = static_cast<int>(get_cache_line_size(L1_CACHE_ID) / sizeof(uint8_t));
    ...
    for (int thread_id = 0; thread_id < num_threads; ++thread_id) {
        threads[thread_id] = std::thread([&, thread_id]() {
            const int block_row_start = std::max(thread_id * block_row_size, center); // to start indexing from center
            const int block_row_end = std::min(src.height - center, (thread_id + 1) * block_row_size);

            for (int rowi = block_row_start; rowi < block_row_end; rowi += block_size) {
                for (int coli = center; coli < src.width - center; coli += block_size) {
                    const int inner_row_end = std::min(block_row_end, rowi + block_size);
                    const int inner_col_end = std::min(src.width - 1, coli + block_size);
                    ...
                    for (int row = rowi; row < inner_row_end; row++) {
                        for (int col = coli; col < inner_col_end; col++) {
                            ...
                            for (int i = 0; i < N; ++i) {
                                for (int j = 0; j < N; ++j) {
                                    const size_t image_row = (row + i - center);
                                    const size_t image_column = (col + j - center);

                                    const size_t index = image_row * src.width + image_column;

                                    const int kernel_index = i * N + j;
